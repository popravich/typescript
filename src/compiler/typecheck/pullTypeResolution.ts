...
module TypeScript {
    ...
    export class PullTypeResolver {
        ...
        // The typeRelationChecker is on the resolver, but later could be moved to the semanticInfoChain
        // if it needs to be used in more places than just the resolver.
        private typeRelationChecker = new TypeRelationChecker(this, this.semanticInfoChain);
        ...
        public cachedFunctionInterfaceType() {
        ...
        public getApparentType(type: PullTypeSymbol): PullTypeSymbol {
        ...
        public getNamedPropertySymbolOfAugmentedType(symbolName: string, parent: PullTypeSymbol): PullSymbol {
            var memberSymbol = parent.getNamedPropertySymbol(symbolName, PullElementKind.SomeValue, this);
            if (memberSymbol) {
                return memberSymbol;
            }
                ...
                if (pathDeclKind & allowedContainerDeclKind) {
                    ...
                    if (declSearchKind & PullElementKind.SomeValue) {
                        ...
                        if (instanceSymbol) {
                            ...
                            childSymbol = instanceType.getNamedPropertySymbol(symbolName, declSearchKind, this);
                    ...
                    var childSymbol = declSymbol.getNamedPropertySymbol(symbolName, declSearchKind, this);
        ...
        private checkInterfaceDeclForIdenticalTypeParameters(interfaceDeclAST: InterfaceDeclarationSyntax, context: PullTypeResolutionContext) {
            ...
            for (var i = 0; i < typeParameters.length; i++) {
                ...
                if (typeParameterAST.constraint) {
                    ...
                    if (!this.typeRelationChecker.typesAreIdenticalWithNewEnclosingTypes(typeParameterConstraint, typeParameterSymbol.getConstraint(), context)) {
                        return false;
                    }
                }
            }
        ...
        private getMemberSymbolOfKind(symbolName: string, kind: PullElementKind, pullTypeSymbol: PullTypeSymbol, enclosingDecl: PullDecl, context: PullTypeResolutionContext) {
            var memberSymbol = pullTypeSymbol.getNamedPropertySymbol(symbolName, kind, this);
            ...
            if (equalsValueClause && (canTypeCheckAST || !contextualType)) {
                ...
                if (!initExprSymbol || !initExprSymbol.type) {
                    ...
                    if (!contextualType) {
                        ...
                        var isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(initTypeSymbol, contextualType, argDeclAST, context, comparisonInfo);
        ...
        private typeCheckVariableDeclaratorOrParameterOrEnumElement(varDeclOrParameter: ISyntaxElement, modifiers: ISyntaxList<ISyntaxToken>, name: ISyntaxToken, typeExpr: ISyntaxElement, init: EqualsValueClauseSyntax, context: PullTypeResolutionContext) {
            ...
            if (hasTypeExpr || init) {
                ...
                if (initTypeSymbol && typeExprSymbol) {
                    ...
                    var isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(initTypeSymbol, typeExprSymbol, varDeclOrParameter, context, comparisonInfo);
            ...
            this.typeCheckCallBacks.push(context => {
                ...
                if (numberIndexSignature && stringIndexSignature &&
                    ...
                    var comparisonInfo = new TypeComparisonInfo();
                        ...
                        if (!this.typeRelationChecker.sourceIsAssignableToTarget(numberIndexSignature.returnType, stringIndexSignature.returnType, funcDeclAST, context, comparisonInfo)) {
                    ...
                    if (name || (member.kind === PullElementKind.Property && name === "")) {
                        ...
                        if (parentSymbol !== allMembers[i].getContainer()) {
                            ...
                            if (indexerKindMatchesMemberNameKind || onlyStringIndexerIsPresent) {
                                var comparisonInfo = new TypeComparisonInfo();
                                if (!this.typeRelationChecker.sourceIsAssignableToTarget(allMembers[i].type, signature.returnType, funcDeclAST, context, comparisonInfo, /*isComparingInstantiatedSignatures*/ false)) {
        ...
        private resolveAccessorDeclaration(funcDeclAst: ISyntaxElement, context: PullTypeResolutionContext): PullSymbol {
            ...
            if (accessorSymbol.isResolved) {
                ...
                if (hasGetter && hasSetter) {
                    ...
                    if (!this.typeRelationChecker.typesAreIdentical(setterSuppliedTypeSymbol, getterSuppliedTypeSymbol, context)) {
        ...
        private typeCheckLogicalOperation(binex: BinaryExpressionSyntax, context: PullTypeResolutionContext) {
            ...
            // other operand type. 
            var comparisonInfo = new TypeComparisonInfo();
            if (!this.typeRelationChecker.sourceIsAssignableToTarget(leftType, rightType, binex, context, comparisonInfo) &&
                !this.typeRelationChecker.sourceIsAssignableToTarget(rightType, leftType, binex, context, comparisonInfo)) {
        ...
        private typeCheckInstanceOfExpression(binaryExpression: BinaryExpressionSyntax, context: PullTypeResolutionContext) {
            ...
            var isValidRHS = this.isAnyOrEquivalent(rhsType) || this.typeRelationChecker.typeIsAssignableToFunction(rhsType, binaryExpression, context);
        ...
        private typeCheckReturnExpression(expression: ISyntaxElement, expressionType: PullTypeSymbol, enclosingFunction: PullDecl, context: PullTypeResolutionContext): void {
            ...
            if (enclosingFunction && enclosingFunction.kind === PullElementKind.ConstructorMethod) {
                ...
                if (classDecl) {
                    ...
                    var comparisonInfo = new TypeComparisonInfo();
                    var isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(expressionType, classSymbol.type, expression, context, comparisonInfo);
                    if (!isAssignable) {
            ...
            if (enclosingFunction) {
                ...
                if (typeAnnotation || enclosingFunction.kind === PullElementKind.GetAccessor) {
                    ...
                    if (expressionType && sigReturnType) {
                        ...
                        var isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(expressionType, sigReturnType, expression, context, comparisonInfo);
        ...
        private typeCheckSwitchStatement(ast: SwitchStatementSyntax, context: PullTypeResolutionContext) {
            ...
            for (var i = 0, n = ast.switchClauses.childCount(); i < n; i++) {
                ...
                if (switchClause.kind() === SyntaxKind.CaseSwitchClause) {
                    ...
                    var comparisonInfo = new TypeComparisonInfo();
                    if (!this.typeRelationChecker.sourceIsAssignableToTarget(expressionType, caseClauseExpressionType, caseSwitchClause.expression, context, comparisonInfo) &&
                        !this.typeRelationChecker.sourceIsAssignableToTarget(caseClauseExpressionType, expressionType, caseSwitchClause.expression, context, comparisonInfo)) {
            ...
            if (!nameSymbol) {
                ...
                if (lhsType.kind === PullElementKind.DynamicModule) {
                    ...
                    if (associatedInstance) {
                        ...
                        nameSymbol = instanceType.getNamedPropertySymbol(rhsName, PullElementKind.SomeValue, this);
                    }
                }
                // could be a module instance
                    ...
                    if (associatedType && !associatedType.isClass()) {
                        nameSymbol = associatedType.getNamedPropertySymbol(rhsName, PullElementKind.SomeValue, this);
                    }
                }
                ...
                if (typeConstraint) {
                    ...
                    if (context.canTypeCheckAST(genericTypeAST)) {
                        this.typeCheckCallBacks.push(context => {
                            ...
                            if (!this.typeRelationChecker.sourceIsAssignableToTarget(typeArg, typeConstraint, genericTypeAST, context)) {
        ...
        private computeQualifiedName(dottedNameAST: QualifiedNameSyntax, context: PullTypeResolutionContext): PullTypeSymbol {
            ...
            var childTypeSymbol = <PullTypeSymbol>lhsType.getNamedPropertySymbol(rhsName, memberKind, this);
            ...
            {
                childTypeSymbol = <PullTypeSymbol>lhsType.getNamedPropertySymbol(rhsName, PullElementKind.SomeValue, this);
            }
            ...
            if (!childTypeSymbol && lhsType.isContainer()) {
                ...
                if (exportedContainer) {
                    childTypeSymbol = <PullTypeSymbol>exportedContainer.getNamedPropertySymbol(rhsName, memberKind, this);
                }
            }
            ...
            if (!childTypeSymbol && enclosingDecl) {
                ...
                if (parentDecl) {
                    ...
                    if (enclosingSymbolType === lhsType) {
                        childTypeSymbol = <PullTypeSymbol>lhsType.getNamedPropertySymbol(rhsName, memberKind, this);//lhsType.findContainedMember(rhsName);
                    }
                }
            }
        ...
        private resolveObjectLiteralMembers(
            ...
            for (var i = 0, len = objectLiteralMembers.nonSeparatorCount(); i < len; i++) {
                ...
                if (objectLiteralContextualType) {
                    assigningSymbol = objectLiteralContextualType.getNamedPropertySymbol(memberSymbol.name, PullElementKind.SomeValue, this);
        ...
        public getBothKindsOfIndexSignaturesIncludingAugmentedType(enclosingType: PullTypeSymbol, context: PullTypeResolutionContext): IndexSignatureInfo {
            return this._getBothKindsOfIndexSignatures(enclosingType, context, /*includeAugmentedType*/ true);
        }

        public getBothKindsOfIndexSignaturesExcludingAugmentedType(enclosingType: PullTypeSymbol, context: PullTypeResolutionContext): IndexSignatureInfo {
        ...
        public _addUnhiddenSignaturesFromBaseType(
            ...
            for (var i = 0; i < baseTypeSignatures.length; i++) {
                ...
                var signatureIsHidden = ArrayUtilities.any(derivedTypeSignatures, sig =>
                    this.typeRelationChecker.signaturesAreIdenticalWithNewEnclosingTypes(baseSignature, sig, context, /*includingReturnType*/ false));
        ...
        private conditionExpressionTypesAreValid(leftType: PullTypeSymbol, rightType: PullTypeSymbol, expressionType: PullTypeSymbol, isContextuallyTyped: boolean, context: PullTypeResolutionContext): boolean {
            if (isContextuallyTyped) {
                var contextualType = context.getContextualType();
                if (this.typeRelationChecker.typesAreIdentical(expressionType, leftType, context) ||
                    this.typeRelationChecker.typesAreIdentical(expressionType, rightType, context) ||
                    this.typeRelationChecker.typesAreIdentical(expressionType, contextualType, context)) {
        ...
        private computeInvocationExpressionSymbol(callEx: InvocationExpressionSyntax, context: PullTypeResolutionContext, additionalResults: PullAdditionalCallResolutionData): PullSymbol {
                    ...
                    if (explicitTypeArgs) {
                        for (var j = 0; j < typeParameters.length; j++) {
                            ...
                            if (typeConstraint) {
                                ...
                                // A type argument satisfies a type parameter constraint if the type argument is assignable
                                // to(section 3.8.4) the constraint type once type arguments are substituted for type parameters.
                                if (!this.typeRelationChecker.sourceIsAssignableToTarget(inferredOrExplicitTypeArgs[j], typeConstraint, targetAST, context, /*comparisonInfo:*/ null, /*isComparingInstantiatedSignatures:*/ true)) {
            ...
            if (!signatures.length) {
                ...
                if (!couldNotFindGenericOverload) {
                    // if there are no call signatures, but the target is assignable to 'Function', return 'any'
                    if (this.cachedFunctionInterfaceType() && this.typeRelationChecker.sourceIsAssignableToTarget(targetTypeSymbol, this.cachedFunctionInterfaceType(), targetAST, context)) {
                        if (callEx.argumentList.typeArgumentList) {
                            ...
                            if (explicitTypeArgs) {
                                for (var j = 0; j < typeParameters.length; j++) {
                                    ...
                                    if (typeConstraint) {
                                        ...
                                        // A type argument satisfies a type parameter constraint if the type argument is assignable
                                        // to(section 3.8.4) the constraint type once type arguments are substituted for type parameters.
                                        if (!this.typeRelationChecker.sourceIsAssignableToTarget(inferredOrExplicitTypeArgs[j], typeConstraint, targetAST, context, null, /*isComparingInstantiatedSignatures:*/ true)) {
                                            var enclosingSymbol = this.getEnclosingSymbolForAST(targetAST);
                                            constraintDiagnostic = this.semanticInfoChain.diagnosticFromAST(targetAST, DiagnosticCode.Type_0_does_not_satisfy_the_constraint_1_for_type_parameter_2, [inferredOrExplicitTypeArgs[j].toString(enclosingSymbol, /*useConstraintInName*/ true), typeConstraint.toString(enclosingSymbol, /*useConstraintInName*/ true), typeParameters[j].toString(enclosingSymbol, /*useConstraintInName*/ true)]);
                                            couldNotAssignToConstraint = true;
                                        }
        ...
        private typeCheckCastExpression(assertionExpression: CastExpressionSyntax, context: PullTypeResolutionContext, typeAssertionType: PullTypeSymbol): void {
            ...
            // would cause.
            var isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(exprType, typeAssertionType, assertionExpression, context, comparisonInfo);

            if (!isAssignable) {
                var widenedExprType = exprType.widenedType(this, assertionExpression.expression, context);
                isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(typeAssertionType, widenedExprType, assertionExpression, context, comparisonInfo);
            }
        ...
        private typeIsBestCommonTypeCandidate(candidateType: PullTypeSymbol, collection: IPullTypeCollection, context: PullTypeResolutionContext): boolean {
            for (var i = 0; i < collection.getLength(); i++) {
                ...
                if (!this.typeRelationChecker.sourceIsSubtypeOfTarget(otherType, candidateType, /*ast*/ null, context)) {
                    return false;
                }
            }
        ...
        private overloadIsApplicableForArgumentHelper(paramType: PullTypeSymbol,
            ...
            if (!context.hasProvisionalErrors() && this.typeRelationChecker.sourceIsSubtypeOfTarget(argSym.type, paramType, arg, context, tempComparisonInfo)) {
                return OverloadApplicabilityStatus.Subtype;
            }
            ...
            if (this.typeRelationChecker.sourceIsAssignableToTarget(argSym.type, paramType, arg, context, comparisonInfo.message ? tempComparisonInfo : comparisonInfo)) {
        ...
        private inferArgumentTypesForSignature(argContext: TypeArgumentInferenceContext, comparisonInfo: TypeComparisonInfo, context: PullTypeResolutionContext): PullTypeSymbol[] {
            ...
            for (var i = 0; i < inferenceResultTypes.length; i++) {
                if (typeParameters[i].getConstraint()) {
                    ...
                    if (!this.typeRelationChecker.sourceIsAssignableToTargetWithNewEnclosingTypes(inferenceResultTypes[i], associatedConstraint, /*ast*/ null, context, /*comparisonInfo*/ null, /*isComparingInstantiatedSignatures*/ false)) {
        ...
        private relateObjectTypeToTypeParameters(objectType: PullTypeSymbol,
            ...
            for (var i = 0; i < parameterTypeMembers.length; i++) {
                objectMember = objectType.getNamedPropertySymbol(parameterTypeMembers[i].name, PullElementKind.SomeValue, this);
                if (objectMember) {
        ...
        private validateVariableDeclarationGroups(enclosingDecl: PullDecl, context: PullTypeResolutionContext) {            
            this.scanVariableDeclarationGroups(
                ...
                (subsequentDecl: PullDecl, firstSymbol: PullSymbol) => {
                    ...
                    if (symbolType && firstSymbolType && symbolType !== firstSymbolType && !this.typeRelationChecker.typesAreIdentical(symbolType, firstSymbolType, context)) {
            ...
            if (isConstantOverloadSignature) {
                if (signature.isDefinition()) {
                    ...
                    for (var i = 0; i < allSignatures.length; i++) {
                        ...
                        if (this.typeRelationChecker.signatureIsAssignableToTarget(signature, allSignatures[i], /*ast*/ null, context)) {
                ...
                if (!this.typeRelationChecker.signatureIsAssignableToTarget(definitionSignature, signature, funcDecl, context, comparisonInfo)) {
                ...
                for (var i = 0; i < members.length; i++) {
                    ...
                    if ((member.name || (member.kind === PullElementKind.Property && member.name === "")) &&
                        ...
                        if (relevantSignature) {
                            var comparisonInfo = new TypeComparisonInfo();
                            if (!this.typeRelationChecker.sourceIsAssignableToTarget(memberSymbol.type, relevantSignature.returnType, member.ast(), context, comparisonInfo)) {
        ...
        private typeCheckIfTypeExtendsType(
            ...
            for (var i = 0; i < typeMembers.length; i++) {
                ...
                if (extendedTypeProp) {
                    ...
                    if (!foundError1) {
                        foundError2 = !this.typeRelationChecker.sourcePropertyIsAssignableToTargetProperty(typeSymbol, extendedType, typeMembers[i], extendedTypeProp, classOrInterface, context, comparisonInfo);
                    }
            ...
            if (!foundError && typeSymbol.hasOwnCallSignatures()) {
                foundError = !this.typeRelationChecker.sourceCallSignaturesAreAssignableToTargetCallSignatures(typeSymbol, extendedType, classOrInterface, context, comparisonInfo);
            }

            // Check construct signatures
            if (!foundError && typeSymbol.hasOwnConstructSignatures()) {
                foundError = !this.typeRelationChecker.sourceConstructSignaturesAreAssignableToTargetConstructSignatures(typeSymbol, extendedType, classOrInterface, context, comparisonInfo);
            }

            // Check index signatures
            if (!foundError && typeSymbol.hasOwnIndexSignatures()) {
                foundError = !this.typeRelationChecker.sourceIndexSignaturesAreAssignableToTargetIndexSignatures(typeSymbol, extendedType, classOrInterface, context, comparisonInfo);
            }
                ...
                if (typeConstructorTypeMembers.length) {
                    ...
                    for (var i = 0; i < typeConstructorTypeMembers.length; i++) {
                        ...
                        if (extendedConstructorTypeProp) {
                            ...
                            if (!this.typeRelationChecker.sourcePropertyIsAssignableToTargetProperty(typeConstructorType, extendedConstructorType, typeConstructorTypeMembers[i], extendedConstructorTypeProp, classOrInterface, context, comparisonInfo)) {
                                foundError = true;
                                break;
                            }
                        }
        ...
        private typeCheckIfClassImplementsType(
            ...
            var comparisonInfo = new TypeComparisonInfo();
            var foundError = !this.typeRelationChecker.sourceMembersAreAssignableToTargetMembers(classSymbol, implementedType, classDecl, context, comparisonInfo);
            if (!foundError) {
                foundError = !this.typeRelationChecker.sourceCallSignaturesAreAssignableToTargetCallSignatures(classSymbol, implementedType, classDecl, context, comparisonInfo);
                if (!foundError) {
                    foundError = !this.typeRelationChecker.sourceConstructSignaturesAreAssignableToTargetConstructSignatures(classSymbol, implementedType, classDecl, context, comparisonInfo);
                    if (!foundError) {
                        foundError = !this.typeRelationChecker.sourceIndexSignaturesAreAssignableToTargetIndexSignatures(classSymbol, implementedType, classDecl, context, comparisonInfo);
                    }
                }
            }
            ...
            for (var i = 0; i < baseMembers.length; i++) {
                ...
                if (inheritedMembersMap[memberName]) {
                    ...
                    if (prevMember.baseOrigin !== baseTypeSymbol &&
                        !this.typeRelationChecker.propertiesAreIdenticalWithNewEnclosingTypes(baseTypeSymbol, prevMember.baseOrigin, member, prevMember.memberSymbol, context)) {
                ...
                if (parameterTypeIsString) {
                    if (allInheritedSignatures.stringSignatureWithBaseOrigin) {
                        if (allInheritedSignatures.stringSignatureWithBaseOrigin.baseOrigin !== baseTypeSymbol &&
                            !this.typeRelationChecker.typesAreIdentical(allInheritedSignatures.stringSignatureWithBaseOrigin.signature.returnType, currentInheritedSignature.returnType, context)) {
                    ...
                    if (allInheritedSignatures.numberSignatureWithBaseOrigin) {
                        if (allInheritedSignatures.numberSignatureWithBaseOrigin.baseOrigin !== baseTypeSymbol &&
                            !this.typeRelationChecker.typesAreIdentical(allInheritedSignatures.numberSignatureWithBaseOrigin.signature.returnType, currentInheritedSignature.returnType, context)) {
        ...
        private checkInheritedMembersAgainstInheritedIndexSignatures(
            ...
            for (var memberName in inheritedMembers) {
                ...
                var memberIsSubtype = this.typeRelationChecker.sourceIsAssignableToTarget(memberWithBaseOrigin.memberSymbol.type, relevantSignature.returnType, interfaceName, context, comparisonInfo);
        ...
        private checkThatInheritedNumberSignatureIsSubtypeOfInheritedStringSignature(
            ...
            if (inheritedIndexSignatures.numberSignatureWithBaseOrigin && inheritedIndexSignatures.stringSignatureWithBaseOrigin) {
                ...
                var comparisonInfo = new TypeComparisonInfo();
                var signatureIsSubtype = this.typeRelationChecker.sourceIsAssignableToTarget(
        ...
        private checkAssignability(ast: ISyntaxElement, source: PullTypeSymbol, target: PullTypeSymbol, context: PullTypeResolutionContext): void {
            ...
            var isAssignable = this.typeRelationChecker.sourceIsAssignableToTarget(source, target, ast, context, comparisonInfo);
        ...
        public getEnclosingSymbolForAST(ast: ISyntaxElement) {
            var enclosingDecl = this.getEnclosingDeclForAST(ast);
