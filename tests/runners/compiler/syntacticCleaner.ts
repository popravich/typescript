...
class SyntacticCleaner extends TypeScript.SyntaxWalker {
    ...
    static clean(sourceFileContents: string): string {
        var parsed = TypeScript.Parser.parse('_emitted.ts', TypeScript.SimpleText.fromString(sourceFileContents), false, new TypeScript.ParseOptions(TypeScript.LanguageVersion.EcmaScript5, true));
