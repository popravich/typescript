...
var loggedIO = path.join(builtLocalDirectory, 'loggedIO.js');
compileFile(loggedIO, ['tests/runners/rwc/loggedIO.ts'], [], [], false);

var instrument = path.join(builtLocalDirectory, 'instrument.js');
compileFile(instrument, ['tests/runners/rwc/instrumentation/instrument.ts'], [], [], false);

desc("Instruments the compiler for IO recording");
task("io-record", [tscFile, loggedIO, instrument], function() {
    host = process.env.host || process.env.TYPESCRIPT_HOST || "node";
    var cmd = host + ' ' + instrument + ' record tscrecord' + tscFile;
    console.log(cmd);
    var ex = jake.createExec([cmd]);
